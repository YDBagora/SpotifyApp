    ...
    <>
      <Box sx={{ px: 2 }}>
        <Typography id="modal-modal-title" variant="h5" component="h1">
          <b className='Title'>Popular Albums and singles</b>
          {visible1 < data1.length && (
            <LanguageBtn onClick={loadData1}><b>Show All</b></LanguageBtn>
          )}
        </Typography>
      </Box>
      {data1.slice(0, visible1).map((item) => {
        return (
          <Gridcontent>
            <Box sx={{ flexGrow: 1 }}>
              <Styles key={item.id}>
                <CardContent>
                  ...
                  <img src={item.image} alt={item.title} className='logoimg1' />
                  <Typography variant="h6" component="h6" sx={{ fontSize: '16px' }}>
                    <b>{item.title.length > 14 ? item.title.slice(0, 14) + '...' : item.title}</b>
                  </Typography>
                  <small>{item.singers}</small>
                </CardContent>
              </Styles>
            </Box>
          </Gridcontent >
